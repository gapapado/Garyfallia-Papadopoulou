...
A, a, b, c, x, y, z, ha, hb, hc, p , ma, mb, mc = symbols('A a b c x y z ha hb hc p ma mb mc')
...
print("A = ", Area, '\n')
'''
G = [2*A-a*ha, 2*A-b*hb,2*A-c*hc,
         A**2 - p*(p - a)*(p - b)*(p - c), 2*p - (a + b + c), ma**2 - (2*b**2 + 2*c**2 - a**2) / 4, mb**2 - (2*a**2 + 2*c**2 - b**2) / 4, mc**2 - (2*a**2 + 2*b**2 - c**2) / 4  ]
basis = groebner(G, *[a, b, c, p, ha, hb, hc, ma, mb, mc, A], order='lex')
print(len(basis), '\n')
print('part_b', basis, '\n')
