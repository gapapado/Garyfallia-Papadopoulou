...
int main(int argc, char **argv) {
	...
	char* bytes = strtok(getenv("QUERY_STRING"), "=");
	char* bytes1 = strtok(NULL, "=");
	int bytes_for_transfer = atoi(bytes1);

	int master_socket, new_socket, client_socket[MAX_CLIENTS], client_reads[MAX_CLIENTS], max_clients = MAX_CLIENTS, i, bytes_read = 0, total_bytes_read = 0, sd;
	int max_sd;
	...
	if (bind(master_socket, (struct sockaddr *) &address, sizeof(address)) < 0) {
		cout << "DCCP: Bind failed" << endl;
		exit(EXIT_FAILURE);
	}
	if (listen(master_socket, 5) < 0) {
		//perror("DCCP: Listen failed\n");
		cout << "DCCP: Listen failed" << endl;
		exit(EXIT_FAILURE);
	}
	//printf("DCCP: Listening on port %d\n", SERVER_PORT);
	cout << "DCCP: Listening on port " << SERVER_PORT << endl;
	while (1) {
		...
		if ((select(max_sd + 1, &readfds, NULL, NULL, NULL) < 0)) {
			...
			cout << "DCCP: Select failed\n";
			exit(EXIT_FAILURE);
		}
		// If master socket was selected, we have an incoming connection
		if (FD_ISSET(master_socket, &readfds)) {
			if ((new_socket = accept(master_socket, 0, (socklen_t *) &address)) < 0) {
				//perror("DCCP: Accept failed\n");
				cout << "DCCP: Accept failed\n";
				exit(EXIT_FAILURE);
			}
			//printf("DCCP: New connection: %s:%d\n", inet_ntoa(address.sin_addr), ntohs(address.sin_port));
			cout << "DCCP: New connection: " << inet_ntoa(address.sin_addr) << ":" << ntohs(address.sin_port) << endl;
		...
		for (i = 0; i < max_clients; i++) {
			...
			if (FD_ISSET(sd, &readfds)) {
				...
				if (bytes_read == 0) {
					...
					//cout << "DCCP: Disconnected: " << inet_ntoa(address.sin_addr) << ":" << ntohs(address.sin_port) << " total " << client_reads[i] << "bytes\n";
					close(sd);
					client_socket[i] = 0;
					client_reads[i] = 0;
				} else {
					printf("DCCP: Read %d bytes from %s:%d\n", bytes_read, inet_ntoa(address.sin_addr), ntohs(address.sin_port));
					//cout << "DCCP: Read " << bytes_read << " bytes from " << inet_ntoa(address.sin_addr) << ":" << ntohs(address.sin_port) << endl;
					client_reads[i] += bytes_read;
				}
			}
		}
		// if transfer is complete, then exit
		if(total_bytes_read >= bytes_for_transfer)
			break;
	}
	return 0;
}
